#!/usr/bin/ruby
unless ARGV.size == 3
  puts "Usage is minute_mtr.rb <DESTINATION> <FILE_DEST> <MAX DAYS>"
  puts "Example minute_mtr.rb google.com /var/tmp/ 7"
  exit 1
end
...
while true do
  ...
  Dir.chdir(fsdest)
  Dir.glob('*.txt').each { |filename| File.delete(filename) if f_age(filename) > max_age }
  sleep 60
end
